...
def download_coco2014(root, phase):
    if not os.path.exists(root):
        os.makedirs(root)
    tmpdir = os.path.join(root, 'COCO2014/tmp/')
    data = os.path.join(root, 'COCO2014/')
    ...
    if not os.path.exists(cached_file):
        ...
        os.chdir(root)
    # extract file
    img_data = os.path.join(data, filename.split('.')[0])  # D:\file\study\data/data/train2014(or val2014)
...
class COCO2014(data.Dataset):
    ...
    def get_anno(self):
        list_path = os.path.join(self.root, 'COCO2014', '{}_anno.json'.format(self.phase))
        self.img_list = json.load(open(list_path, 'r'))
        self.cat2idx = json.load(open(os.path.join(self.root, 'COCO2014', 'category.json'), 'r'))
    ...
    def get(self, item):
        ...
        img = Image.open(os.path.join(self.root, 'COCO2014', '{}2014'.format(self.phase), filename)).convert('RGB')
