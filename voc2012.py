...
def read_object_labels(root, dataset, set):
    path_labels = os.path.join(root, 'VOCdevkit2012', dataset, 'ImageSets', 'Main')
    labeled_data = dict()
    num_classes = len(object_categories)  # 20

    for i in range(num_classes):
        file = os.path.join(path_labels, object_categories[i] + '_' + set + '.txt')
        data = read_image_label(file)

        # print(data.items())  # dict_items([('000005', -1), ('000007', -1), ('000009', -1), ('000012', -1)])
    ...
    with open(file, 'r') as f:
        ...
        for row in reader:
            if header and rownum == 0:
                ...
                if num_categories == 0:
                    num_categories = len(row) - 1  # num_categories: 20
                name = row[0]
...
def download_voc2012(root):
    path_devkit = os.path.join(root, 'VOCdevkit2012')
    path_images = os.path.join(root, 'VOCdevkit2012', 'VOC2012', 'JPEGImages')
    ...
    # test images
    test_image = os.path.join(path_devkit, 'VOC2012/JPEGImages/2007_000027.jpg')
    if not os.path.exists(test_image):

        # download test images
        parts = urlparse(urls['test_anno_2012'])
        filename = os.path.basename(parts.path)
        cached_file = os.path.join(tmpdir, filename)

        if not os.path.exists(cached_file):
            print('Downloading: "{}" to {}\n'.format(urls['test_anno_2012'], cached_file))
            util.download_url(urls['test_anno_2012'], cached_file)
...
class Voc2012Classification(data.Dataset):
    def __init__(self, root, set, transform=None, target_transform=None, inp_name=None):
        self.root = root
        self.path_devkit = os.path.join(root, 'VOCdevkit2012')  # D:\file\study\data/VOCdevkit
        self.path_images = os.path.join(root, 'VOCdevkit2012', 'VOC2012', 'JPEGImages')
        self.set = set
        ...
        path_csv = os.path.join('./files', 'VOC2012')
        ...
        # Initialize a list to store text features
        text_features = []

        # Loop through object categories and get text features
        for category in object_categories:
            text_inputs = torch.cat([clip.tokenize(f"a photo of a {category}")]).to("cuda")
            with torch.no_grad():
                text_feature = model.encode_text(text_inputs).float()
            text_features.append(text_feature.cpu().numpy())

        text_features_matrix = np.array(text_features)
        text_features_tensor = torch.tensor(text_features_matrix)
        self.inp = text_features_tensor.squeeze()
